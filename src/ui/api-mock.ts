import {
	ZhivaApiOptions,
	ZhivaApiQuery,
} from "@wxn0brp/zhiva-base-lib/types/api";

export async function mockFetchApi(
	endpoint: string,
	options: ZhivaApiOptions = {},
	query: ZhivaApiQuery = {},
): Promise<Response> {
	console.log(`[BROWSER MODE] Mocked API call for: ${endpoint}`, {
		options,
		query,
	});
	...
	switch (endpoint) {
		...
		case "update":
			console.warn(
				`Action "${endpoint}" is not available in browser-only mode. Doing nothing.`,
			);
			return new Response(
				JSON.stringify({
					err: false,
		...
		default:
			...
			return new Response(
				JSON.stringify({
					err: true,
					msg: "Endpoint not mocked for browser mode",
				}),
				{
					status: 404,
					headers: {
...
async function fetchApps() {
	...
	const banned: string[] = await getFromCache(
		"banned",
		5 * 60 * 1000,
		fetchBanned,
	);
...
async function fetchAllRepos(): Promise<Repo[]> {
	const data = await fetch(
		"https://api.github.com/search/repositories?q=topic:Zhiva-app",
	).then(res => res.json());
...
async function fetchData(file: string) {
	const data = await fetch(
		`https://raw.githubusercontent.com/wxn0brP/Zhiva-registry/HEAD/${file}.txt`,
	).then(res => res.text());
	return data
		.split("\n")
